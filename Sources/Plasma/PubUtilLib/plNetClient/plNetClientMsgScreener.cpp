...
bool plNetClientMsgScreener::AllowOutgoingMessage(const plMessage* msg) const
{
    ...
    if (ans==kNo)
    {
        WarningMsg(plFormat("Rejected: (Outgoing) {} [Illegal Message]", msg->ClassName()));
        return false;
    }

    if (!IValidateMessage(msg))
    {
        WarningMsg(plFormat("Rejected: (Outgoing) {} [Validation Failed]", msg->ClassName()));
        return false;
    }
    return true;
}
...
bool plNetClientMsgScreener::AllowIncomingMessage(const plMessage* msg) const
{
    ...
    if (!result)
        WarningMsg(plFormat("Rejected: (Incoming) {}", msg->ClassName()));
