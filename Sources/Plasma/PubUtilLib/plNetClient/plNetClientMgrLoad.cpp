...
plKey plNetClientMgr::ILoadClone(plLoadCloneMsg *pCloneMsg)
{
    ...
    if(pCloneMsg->GetIsLoading())
    {
        ...
        {
            DebugMsg(plFormat("ILoadClone: object {} is already loaded, ignoring", cloneKey->GetUoid().StringIze()));
            return cloneKey;
        }
    ...
    else        // we're unloading a clone
    {
        if (!cloneKey->ObjectIsLoaded())
        {
            DebugMsg(plFormat("ILoadClone: object {} is already unloaded, ignoring", cloneKey->GetName()));
            return cloneKey;
        }
...
void plNetClientMgr::IPlayerChangeAge(bool exitAge, int32_t spawnPt)
{
    ...
    {
        ErrorMsg(plFormat("Can't find avatarMod {}", fLocalPlayerKey->GetName()));
    }
}
