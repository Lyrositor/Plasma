...
plAgeLoader::~plAgeLoader()
{
    ...
    if ( PendingAgeFniFiles().size() )
        plNetClientApp::StaticErrorMsg(plFormat("~plAgeLoader(): {} pending age fni files", PendingAgeFniFiles().size()));
    if ( PendingPageOuts().size() )
        plNetClientApp::StaticErrorMsg(plFormat("~plAgeLoader(): {} pending page outs", PendingPageOuts().size()));
...
bool plAgeLoader::ILoadAge(const plString& ageName)
{
    ...
    nc->DebugMsg(plFormat("Net: Loading age {}", fAgeName));
    ...
    */
    ...
    if (!stream)
    {
        nc->ErrorMsg(plFormat("Failed loading age.  Age desc file {} has nil stream", fAgeName));
        fFlags &= ~kLoadingAge;
        return false;
    }
    ...
    while( ( page = ad.GetNextPage() ) != nil )
    {
        if( IsPageExcluded( page, fAgeName) )
        {
            nc->DebugMsg(plFormat("\tExcluding page {}\n", page->GetName()));
            continue;
        }
        ...
        nc->DebugMsg(plFormat("\tPaging in room {}\n", page->GetName()));
    }
...
{
    plNetClientApp* nc = plNetClientApp::GetInstance();
    nc->DebugMsg(plFormat("Net: Unloading age {}", fAgeName));
    ...
    for( i = 0; i < collector.fPages.GetCount(); i++ )
    {
        ...
        {
            nc->DebugMsg(plFormat("\tPaging out room {}\n", page->GetPageInfo().GetPage()));
