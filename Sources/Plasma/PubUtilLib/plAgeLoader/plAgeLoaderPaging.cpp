...
bool ReportRoomToServer(const plKey &key)
{
    ...
    if (skip)
        hsLogEntry(plNetApp::StaticDebugMsg(plFormat("Not reporting room {} to server, reserved={}, virtual={}", 
...
void plAgeLoader::FinishedPagingInRoom(plKey* rmKey, int numRms)
{
    ...
    for(i=0;i<numRms;i++)
    {
        ...
        hsLogEntry(nc->DebugMsg(plFormat("\tSending PageIn/RequestState msg, room={}\n", key->GetName())));
    }
...
void plAgeLoader::FinishedPagingOutRoom(plKey* rmKey, int numRms)
{
    ...
    for(i=0;i<numRms;i++)
    {
        ...
        if( found != fPendingPageOuts.end() )
        {
            ...
            nc->DebugMsg(plFormat("Finished paging out room {}", rmKey[i]->GetName()));
        }
    }
...
void plAgeLoader::StartPagingOutRoom(plKey* rmKey, int numRms)
{
    ...
    for(i=0;i<numRms;i++)
    {
        ...
        nc->DebugMsg(plFormat("\tSending PageOut msg, room={}", rmKey[i]->GetName()));
    }
...
void plAgeLoader::IgnorePagingOutRoom(plKey* rmKey, int numRms)
{
    ...
    for(i=0;i<numRms;i++)
    {
        ...
        {
            fPendingPageOuts.erase( found );
            nc->DebugMsg(plFormat("Ignoring paged out room {}", rmKey[i]->GetName()));
        }
    }
